  ...
  actions: {
    ...
    async searchYearsAndStations(provider?: string): Promise<void> {
      try {
        ...
        }>>(`https://get-years-stations-sm6mx5mo3a-uc.a.run.app/?fileEndsWith=${fileEndsWith ?? 'mat'}`)

        this.years = new Set<number>()
        this.receivers = new Map<number, Set<string>>()

        response.data.forEach((value: { year: number; stations: Array<string> }) => {
    ...
    async searchFiles(
      ...
      try {
        const response = await axios.get<Array<File>>(
          `https://get-available-files-sm6mx5mo3a-uc.a.run.app/?year=${year}&month=${date.getMonth() + 1}&day=${date.getDate()}&station=${station}&type=${type.toLowerCase()}&fileEndsWith=${fileEndsWith ?? 'mat'}`,
        )
